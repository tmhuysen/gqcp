            ...
            for (size_t r = 0; r < this->K; r++) {
                for (size_t s = 0; s < this->K; s++) {
                    for (size_t t = 0; t < this->K; t++) {
                        F(p,q) += g->operator()(q,r,s,t) * d(p,r,s,t);
                    }
                }
            }  // two-electron part
        ...
        for (size_t q = 0; q < this->K; q++) {
            for (size_t r = 0; r < this->K; r++) {
                for (size_t s = 0; s < this->K; s++) {
                    ...
                    // One-electron part
                    W(p,q,r,s) -= this->h->operator()(s,p) * D(r,q);

                    // Two-electron part
                    for (size_t t = 0; t < this->K; t++) {
                        for (size_t u = 0; u < this->K; u++) {
                            W(p,q,r,s) += this->get_g()(s,t,q,u) * d(r,t,p,u) - this->g->operator()(s,t,u,p) * d(r,t,u,q) - this->g->operator()(s,p,t,u) * d(r,q,t,u);
                        }
                    }  // two-electron part
                }
            }
