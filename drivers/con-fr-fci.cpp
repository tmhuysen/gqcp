...
// Print the energy to an output file
    ...
    // Solve the FFCI eigenvalue problem using the dense algorithm
    auto K = mol_ham_par.get_K();

    GQCP::ProductFockSpace fock_space (K, N_alpha, N_beta);
    GQCP::ProductFockSpace fock_space_x (K-2, N_alpha-2, N_beta-2);
    GQCP::FFCI fci (fock_space, 2);
    GQCP::CISolver ci_solver (fci, mol_ham_par);
    GQCP::DavidsonSolverOptions davidson_solver_options(fock_space_x.HartreeFockExpansion());
    ...
    GQCP::FFCIRDMBuilder fci_rdm (fock_space, 2);
    ...
    for (size_t i = 0; i < lambdasv.rows(); i++) {
        ...
        try {
            ...
            std::cout << "\033[1;31m DAVIDSON FAILED IN: \033[0m" << lin;
            continue;
        }
